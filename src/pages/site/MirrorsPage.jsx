...
export default function MirrorsPage() {
  ...
  useEffect(() => {
    const fetchMirrors = async () => {
      try {
        const response = await axios.get(`${apiUrl}/products/mirrors`);
        handleSortChange("");
        handleSortChange("name_asc");
        setProducts(response.data);
      } catch (error) {
        console.error("Грешка при извличане на огледалата:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const indexOfLastMirror = currentPage * productsPerPage;
  const indexOfFirstMirror = indexOfLastMirror - productsPerPage;
  const currentMirrors = products.slice(indexOfFirstMirror, indexOfLastMirror);
        ...
        <div>
          <div className="products-grid-container">
            {currentMirrors.map((mirror) => (
              <ProductCard product={mirror} key={mirror.product_id} />
            ))}
          </div>
          <PaginationButtons
            productsPerPage={productsPerPage}
