...
router.get('/new-products', async (req, res) => {
  try {
    ...
    const [rows] = await connection.query('SELECT * FROM products WHERE product_release_date >= DATE_SUB(NOW(), INTERVAL 7 DAY)');
    connection.release();
...
router.get('/frames', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Рамки"!' });
  }
});
...
router.get('/profiles', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Профили"!' });
  }
});
...
router.get('/matboards', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Паспарту"!' });
  }
});
...
router.get('/gobelins', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Гоблени"!' });
  }
});
...
router.get('/panels', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Пана"!' });
  }
});
...
router.get('/mirrors', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Огледала"!' });
  }
});
...
router.get('/icons', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Икони"!' });
  }
});
...
router.get('/art-materials', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на продуктите от категория "Арт материали"!' });
  }
});
...
router.get('/details/:productId', async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Грешка при зареждане на данните за продукта!' });
  }
});
...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    if (searchTerm) {
      query += '(product_name LIKE ? OR product_category LIKE ? OR product_material LIKE ?)';
